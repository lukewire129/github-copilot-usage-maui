...
static class AppStrings
{
    ...
    // Token expired
    public static string ClaudeTokenExpired => IsKorean
        ? "⚠ Claude 인증 토큰이 만료되었습니다"
        : "⚠ Claude auth token has expired";
    public static string ClaudeTokenExpiredDesc => IsKorean
        ? "토큰이 만료되어 사용량 데이터를 조회할 수 없습니다.\nClaude CLI로 재인증하거나 사이트에서 로그인하세요."
        : "Cannot fetch usage data because the token has expired.\nRe-authenticate via Claude CLI or log in on the website.";
    public static string ClaudeReauthCli => IsKorean ? "Claude CLI로 재인증" : "Re-auth via CLI";
    public static string ClaudeOpenSite => IsKorean ? "사이트 열기" : "Open Website";
    ...
    public static string ClaudePaceLabel => IsKorean ? "사용 속도" : "Face";
